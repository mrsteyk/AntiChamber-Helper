...
public:
	...
	std::string GetName() const
	{
		...
		//std::string str(length, '\0');
		std::string str((char*)WideName); // what on Earth???

		//std::use_facet<std::ctype<wchar_t>>(std::locale("")).narrow(WideName, WideName + length, '?', &str[0]);
		
		/*auto state = std::mbstate_t();
		auto len = 1 + std::wcsrtombs(nullptr, (const wchar_t**)&WideName, 0, &state);
		
		std::string str(len, '\0');

		std::wcsrtombs(&str[0], (const wchar_t**)&WideName, str.size(), &state);*/

		/*static char tmp[2048]{ 0 };

		sprintf(tmp, "%ls", WideName);
...
struct FName
{
	...
	{
		return GetGlobalNames()[Index]->GetWideName();
	};
